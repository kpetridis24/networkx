...
from networkx.algorithms.isomorphism.vf2pp import vf2pp_isomorphism, vf2pp_monomorphism
...
class TestSubGraphMonoVF2pp:
    def test_first_graph_larger(self):
        G = nx.path_graph(11)
        H = nx.path_graph(10)
        m = vf2pp_monomorphism(G, H)
        assert m is None

    def test_invalid_label_properties(self):
        G = nx.path_graph(2)
        H = nx.path_graph(3)

        G.nodes[0]["label"] = "black"
        G.nodes[1]["label"] = "red"

        H.nodes[0]["label"] = "black"
        H.nodes[1]["label"] = "blue"
        H.nodes[2]["label"] = "green"

        m = vf2pp_monomorphism(G, H, node_label="label")
        assert m is None

    def test_wikipedia_graph_no_labels(self):
        edges1 = [
            ["a", "g"],
            ["a", "h"],
            ["a", "i"],
        ...
        m = vf2pp_monomorphism(G3, G1, node_label=None)
        assert m is not None

    def test_graph_DB_no_labels(self):
        head, tail = os.path.split(__file__)
        subgraph = create_graph(os.path.join(head, "../si2_b06_m200.A99"))
        graph = create_graph(os.path.join(head, "../si2_b06_m200.B99"))
        m = vf2pp_monomorphism(subgraph, graph, node_label=None)
        assert m is not None

    def test_custom_graph1_no_labels(self):
        G1 = nx.Graph()
        ...
        m = vf2pp_monomorphism(g2, G1, node_label=None)
        assert m is not None
        ...
        m = vf2pp_monomorphism(g4, G2, node_label=None)
        assert m is not None
    ...
    def test_custom_graph2_no_labels(self):
        ...
        m = vf2pp_monomorphism(g1, G2, node_label=None)
        assert m is not None
